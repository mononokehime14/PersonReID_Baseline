...
def train(config_file, *args, **kwargs):
    ...
    #Person_ReID_Dataset_Downloader('home/GTA/',cfg.DATASETS.NAMES)
    ...
    train_loader, _, _, num_classes = data_loader(cfg,cfg.DATASETS.NAMES)
    val_loaders = []
    num_queries = []
    for arg in args:
        _, val_loader, num_query, _ = data_loader(cfg, arg, is_train=False)
    ...
    for epoch in range(epochs):
        ...
        if (epoch+1) % eval_period == 0:
            for i, val_loader in enumerate(val_loaders):
                ...
                cmc, mAP = evaluation(all_feats,all_pids,all_camids,num_queries[i])
                logger.info("Validation Results on {} - Epoch: {}".format(args[i], epoch+1))
