...
def train(config_file, **kwargs):
    ...
    model = torch.nn.DataParallel(getattr(models, cfg.MODEL.NAME)(num_classes=num_classes), device_ids  =[0,1])
    ...
    for epoch in range(epochs):
        ...
        for data in tqdm(train_loader, desc='Iteration', leave=False):
            ...
            if cfg.MODEL.NAME == 'DenseNet': 
                results, y, y2 = model(images)
                loss = loss_fn(images, labels, results, y, y2)
            else: 
        ...
        if (epoch+1) % checkpoint_period == 0:
            model.module.cpu()
            model.module.save(output_dir,epoch+1)
        ...
        if (epoch+1) % eval_period == 0:
            ...
            for data in tqdm(val_loader, desc='Feature Extraction', leave=False):
                ...
                with torch.no_grad():
                    ...
                    if cfg.MODEL.NAME == 'DenseNet': 
                        feats = model(images, output_feature='pool5')
                    else: 
                        feats = model(images)
